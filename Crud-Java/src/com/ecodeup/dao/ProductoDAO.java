...
public class ProductoDAO 
{
	...
	public boolean guardar(producto producto) throws SQLException
	{
		...
		connection = obtenerConexion();//Puede generar excepción. La añado al método
		//Utilizo Try Catch por si se produce error, rollback
		try {
			//Utilizamos transacción
			connection.setAutoCommit(false);
			sql = "INSERT INTO productos (id,nombre,cantidad,precio,fecha_crear,fecha_actualizar) "
					+ "VALUES(?,?,?,?,?,?)";
			statement = connection.prepareStatement(sql);
			//Añadimos los parámetros
			statement.setString(1,null);//Id lo asigna el motor de BBDD
			...
			connection.commit();
			statement.close();
			connection.close();//Devuelvo la conexión al pool de conexiones
